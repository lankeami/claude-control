...
- Interactive feature parity: `--allowedTools` → `permissions.allow` in the generated settings file; turn limits → server counts assistant transcript entries and sends ESC; budget caps → server sums per-turn cost from transcript usage (`SessionCostTotal`). Permission prompts bridge remotely via the `PermissionRequest` hook: the generated settings file wires `hook-signal --event permission_request`, which forwards the pending tool call to `/api/sessions/{id}/permission-request` (long-poll), the web UI shows an Allow/Deny card, and the decision returns as hook JSON (`decision.behavior`); on any relay error the hook emits nothing so the TUI dialog takes over (fail-open). Known gap: images are referenced by file path in the prompt instead of inline
