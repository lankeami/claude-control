...
- Interactive feature parity: `--allowedTools` → `permissions.allow` in the generated settings file; turn limits → server counts assistant transcript entries and sends ESC; budget caps → server sums per-turn cost from transcript usage (`SessionCostTotal`). Permission prompts bridge remotely via the `PermissionRequest` hook: the generated settings file wires `hook-signal --event permission_request`, which forwards the pending tool call to `/api/sessions/{id}/permission-request` (long-poll), the web UI shows an Allow/Deny card, and the decision returns as hook JSON (`decision.behavior`); on any relay error the hook emits nothing so the TUI dialog takes over (fail-open). AskUserQuestion is detected in real time via a PreToolUse hook (matcher `AskUserQuestion` → `hook-signal --event question`), not the transcript — the CLI buffers the assistant entry containing a pending question and only flushes it to the transcript JSONL after the question is resolved, so transcript tailing can never surface a live question. The transcript path remains as a deduped fallback (`PendingQuestionManager` tracks pending/resolved tool_use_ids), and a tool_result matching the pending question clears the card (covers questions answered directly in the TUI). Known gap: images are referenced by file path in the prompt instead of inline
